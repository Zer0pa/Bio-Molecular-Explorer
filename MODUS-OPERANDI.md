...
| **Orchestrator** | Synthesis output + repo state | `PRD.md` with overnight-executable specifics, sub-agent allocation, build sequence, interface contracts, falsification ledger, audit-trail spec, acceptance gates | Multi-model routing (Opus 4.7 + GPT-5+ + TxGemma 27B), sub-agents in parallel worktrees, Perplexity / Gemini at stuck points, recursive fresh eyes |
...
- Boundary block (research use only) appears verbatim in every artifact.
...
This applies to every work stream, not just Health.
...
├── briefing-pack/                         # Synthesis agent's primer for downstream readers
...
When a new work stream starts, copy this `MODUS-OPERANDI.md` verbatim, write a stream-specific `HANDOFF-TO-<NEXT-ROLE>.md`, and the synthesis-agent material specific to that stream.
...
- **Heavy code generator** — GPT-5+ (5.4 today, 5.5 when available) at xhigh reasoning. Writes substantive code.
- **Per-layer specialists** — sub-agents in parallel worktrees, one per pipeline layer. Each owns its layer's code, tests, schemas.
- **Domain reasoner** — TxGemma 27B (or domain-fine-tuned successor) for therapeutics-specific queries. CPU-quantised for dev work.
...
1. **Scientific gate** — every layer has falsifier coverage, source grounding, no clinical claim.
